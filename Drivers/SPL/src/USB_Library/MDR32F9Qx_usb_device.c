...
#define LOG_LOCAL_LEVEL LOG_TAG_USBD_LOCAL_LEVEL
#include <mdr_log.h>
static const char *TAG = "USBD";
...
static uint8_t rxBufDbg[64]; // TODO Компилировать буфер только при уровле VERBOSE
        ...
        switch (ep->EP_State)
        {
            ...
            case USB_EP_IDLE:
            {
                if (!(tmpCTRL & USB_SEP_CTRL_EPRDY) && ep->EP_WasScdone)
                {
                    ...
                    {
                        /* Unexpected transaction: switch into NAK state and return error */
                        MDR_LOGE(TAG, "Unexpected transaction in %u Endpoint, SET NAK", EPx);
                        result = USB_ERROR;
                        USB_EP_Idle(EPx);
                    }
                }
            ...
            case USB_EP_OUT:
            {
                ...
                if (!(tmpCTRL & USB_SEP_CTRL_EPRDY) && ep->EP_WasScdone)
                {
                    ...
                    MDR_LOGD(TAG, "USB_EP_OUT[%u] %lu bytes", EPx, count);
                    MDR_LOG_BUFFER_HEXDUMP(TAG, &ep->Buffer.IO_Buffer.pBuffer[ep->Buffer.IO_Buffer.offset - count], count, MDR_LOG_VERBOSE);
            ...
            case USB_EP_SETUP:
            {
                ...
                if (ep->EP_WasScdone)
                {
                    ...
                    if (count == 8)
                    {
                        ...
                        RX_FIFO_FORCE_EMPTY(EPx);
                        MDR_LOGD(TAG, "USB_EP_SETUP[%u] %lu bytes", EPx, count);
                        MDR_LOG_BUFFER_HEXDUMP(TAG, ep->Buffer.pSetupPacket, count, MDR_LOG_VERBOSE);
                    ...
                    {
                        for (i = 0; i < count; i++) {
                            rxBufDbg[i] = EPx_RX_FIFO_DATA(EPx);
                        }
                        RX_FIFO_FORCE_EMPTY(EPx);
                        MDR_LOGE(TAG, "SETUP not 8 bytes: %lu", count);
                        MDR_LOG_BUFFER_HEXDUMP(TAG, rxBufDbg, count, MDR_LOG_VERBOSE);
                        result = USB_ERROR;
...
static void USB_EP_sendInDataPortion(USB_EP_TypeDef EPx, USB_EPData_Bit_TypeDef DataBitChange)
{
    ...
    assert_param((ep->EP_State == USB_EP_IN) || (ep->EP_State == USB_EP_SETUP));
...
USB_Result USB_DeviceSetupPacket(USB_EP_TypeDef EPx, const USB_SetupPacket_TypeDef* USB_SetupPacket)
{
    ...
    MDR_LOGD(TAG, "DeviceSetupPacket. EP=%u, mRequestTypeData=%02Xh, bRequest=%u, wValue=%04Xh, wIndex=%04Xh, wLength=%u",
             EPx,
             USB_SetupPacket->mRequestTypeData,
             USB_SetupPacket->bRequest,
             USB_SetupPacket->wValue,
             USB_SetupPacket->wIndex,
             USB_SetupPacket->wLength);
...
USB_Result USB_DeviceClearFeature(USB_RequestRecipient_TypeDef Recipient, uint16_t wVALUE, uint16_t wINDEX)
{
    ...
    {
        if ((USB_EP_TypeDef)wINDEX != USB_EP0) {
            MDR_LOGE(TAG, "Clear Feature call, but EP is not Control. EP: &u", (USB_EP_TypeDef)wINDEX);
        }
        return USB_EP_Idle(USB_EP0);
    }
    else
    {
        return USB_ERROR;
